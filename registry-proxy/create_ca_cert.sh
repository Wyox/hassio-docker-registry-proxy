...
mkdir -p /data/certs /data/ca
cd /data/ca

CA_KEY_FILE=${CA_KEY_FILE:-/data/ca/ca.key}
CA_CRT_FILE=${CA_CRT_FILE:-/data/ca/ca.crt}
CA_SRL_FILE=${CA_SRL_FILE:-/data/ca/ca.srl}
...
if [ -f "$CA_CRT_FILE" ] ; then
    ...
    logInfo "*** Please *** make sure to mount /data/ca as a volume -- if not, everytime this container starts, it will regenerate the CA and nothing will work."
